...
func RecordErrorLog(c *gin.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeMillis int64,
	...
	if useTimeMillis > 0 {
		...
		other.SetPublic("duration_ms", useTimeMillis)
	}
...
type RecordConsumeLogParams struct {
	...
	// UseTimeMillis is persisted as public other.duration_ms; use_time keeps
	// truncated seconds for legacy readers. Zero means unknown (async tasks).
...
func RecordConsumeLog(c *gin.Context, userId int, params RecordConsumeLogParams) {
	...
	if params.UseTimeMillis > 0 {
		...
		params.Other.SetPublic("duration_ms", params.UseTimeMillis)
	}
