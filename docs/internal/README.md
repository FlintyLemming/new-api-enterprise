...
2026-09-09 同步至 `v1.0.0-rc.36`（`ea7cb0ba4`），合并提交 `a72e8c116`。本地 `main` 已快进至该 tag，内部功能保留 merge 历史；更新前备份为 `backup/internal-custom-before-rc36-20260909`（`d921bc998`）。代码未推送；2026-09-09 已构建并部署 `new-api:rc36-2422942fc` 到 8850 环境，详见 [部署台账 §10](patches/2026-08-15-langfuse-local-deployment.md#10-2026-09-09-rc36-镜像上线)。2026-09-12 并入订阅重置卡（`feat/subscription-reset-card`）与用量统计缓存口径（`feature/usage-stats-cache-caliber`），构建并部署 `new-api:rc36-e6b8fc10d`，容器 healthy，详见 [部署台账 §11](patches/2026-08-15-langfuse-local-deployment.md#11-2026-09-12-重置卡--缓存统计口径镜像上线)。
